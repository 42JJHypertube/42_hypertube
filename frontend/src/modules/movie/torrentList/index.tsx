...
function TorrentList({ imdb_id, torrents }: { imdb_id: string, torrents: torrentInfo[] }) {
    console.log(imdb_id)
    const filteredTorrents = torrents.filter(info => parseQuality(info.quality) <= 2160);
    ...
    return (
        <div>
            ...
            <button onClick={() => postTorrentDownload({imdb_id, magnetUrl})}>다운로드</button>
        </div>
    )
}
